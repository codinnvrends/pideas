...
/**
 * Generate Complete Project using CrewAI - NEW FEATURE
 */
export const generateCompleteProject = onCall({maxInstances: 2, timeoutSeconds: 1800}, async (request: any) => {
  try {
    const { userId, ideaId, projectName, ideaDescription, userProfile } = request.data;
    
    if (!userId || !ideaDescription || !projectName) {
      throw new Error("User ID, project name, and idea description are required");
    }

    logger.info("Starting complete project generation:", { userId, projectName, ideaId });
    ...
    // Spawn Python process for CrewAI execution
    ...
    const projectInputs = {
      ...
      user_skill_level: userProfile?.skillLevel || 'intermediate',
      user_preferences: userProfile?.preferredTechnologies || [],
      user_interests: userProfile?.interests || [],
      team_size: userProfile?.teamSize || 'individual',
      project_duration: userProfile?.projectDuration || '1-2 months'
    };

    // Create initial status document in Firestore
    const db = admin.firestore();
    const statusDoc = {
      generationId,
      userId,
    ...
    await db.collection('projectGenerations').doc(generationId).set(statusDoc);
...
# Add the functions src directory to Python path
sys.path.insert(0, '${__dirname}')

try:
    from crewai.crew import ProjectGenerationManager
    ...
    # Project inputs
    project_inputs = ${JSON.stringify(projectInputs)}
    generation_id = "${generationId}"
    user_id = "${userId}"
    idea_id = "${ideaId || ''}"
    
    # Update status to starting
    db.collection('projectGenerations').document(generation_id).update({
        'status': 'in_progress',
        'currentStage': 'requirements_analysis',
        'progressPercentage': 10
    })
    
    # Create and run CrewAI generation
    manager = ProjectGenerationManager(db)
    result_generation_id = manager.start_generation(project_inputs, user_id, idea_id)
    
    print(f"SUCCESS:{result_generation_id}")
    ...
    try:
        db.collection('projectGenerations').document("${generationId}").update({
            'status': 'failed',
...
`;
    ...
    const scriptPath = `/tmp/crewai_script_${generationId}.py`;
    ...
    // Execute Python script asynchronously and return immediately
    const python = spawn('python3', [scriptPath], { env, detached: true, stdio: 'pipe' });
    ...
    python.on('close', async (code: number | null) => {
      try {
        ...
        logger.error('Error in cleanup:', cleanupError);
      }
    });

    // Return immediately with generation ID for status polling
    return {
      success: true,
      generationId,
      message: "Project generation started successfully",
      status: "initializing",
      estimatedCompletionTime: statusDoc.estimatedCompletionTime
    };

  } catch (error) {
    logger.error("Error in generateCompleteProject:", error);
    throw new Error(`Failed to generate complete project: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
});
 ...
 */
export const getProjectGenerationStatus = onCall({maxInstances: 10}, async (request: any) => {
  try {
    const { generationId, userId } = request.data;
    ...
    const db = admin.firestore();
    const doc = await db.collection('projectGenerations').doc(generationId).get();
    ...
    // Verify user has access to this generation (security check)
    ...
    logger.error("Error getting project generation status:", error);
    throw new Error(`Failed to get status: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
});

/**
 * Get user's project generation history
 */
export const getProjectGenerationHistory = onCall({maxInstances: 5}, async (request: any) => {
  try {
    const { userId } = request.data;
    ...
    const db = admin.firestore();
    const snapshot = await db.collection('projectGenerations')
      .where('userId', '==', userId)
    ...
    logger.error("Error getting project generation history:", error);
    throw new Error(`Failed to get history: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
});
 ...
 */
export const downloadGeneratedProject = onRequest({maxInstances: 10, cors: true}, async (req, res) => {
  try {
    ...
    // Check if generation is completed and has a download URL
    if (data?.status !== 'completed' || !data.result?.finalDeliverable?.downloadUrl) {
      res.status(400).json({ error: "Project not ready for download" });
      return;
    }

    // Get the download URL from Firebase Storage
    const bucket = admin.storage().bucket();
    const filePath = data.result.finalDeliverable.storagePath;
    ...
    const projectTitle = data.result?.finalDeliverable?.projectTitle || 'project';
    const sanitizedTitle = projectTitle.replace(/[^a-zA-Z0-9-_]/g, '_');
    
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', `attachment; filename="${sanitizedTitle}_${generationId.slice(0, 8)}.zip"`);
...
/**
 * Clean up old generated project files (scheduled function)
 */
export const cleanupOldProjects = onCall({maxInstances: 1}, async () => {
  try {
    const db = admin.firestore();
    const bucket = admin.storage().bucket();
    
    // Get generations older than 30 days
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - 30);
    
    const oldGenerations = await db.collection('projectGenerations')
      .where('startedAt', '<', cutoffDate.toISOString())
      .get();
    
    let cleanedCount = 0;
    
    for (const doc of oldGenerations.docs) {
      const data = doc.data();
      
      // Delete from Firebase Storage if exists
      if (data.result?.finalDeliverable?.storagePath) {
        try {
          await bucket.file(data.result.finalDeliverable.storagePath).delete();
        } catch (error) {
          // File might already be deleted, log but continue
          logger.warn(`Failed to delete storage file: ${data.result.finalDeliverable.storagePath}`, error);
        }
      }
    ...
    logger.info(`Cleaned up ${cleanedCount} old project generations`);
    
    return {
      success: true,
      cleanedCount,
      message: `Successfully cleaned up ${cleanedCount} old project generations`
    };
    
  } catch (error) {
    logger.error("Error cleaning up old projects:", error);
